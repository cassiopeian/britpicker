...
$(document).on('keyup change input', '#ae', function() {
    ...
    for (let i = 0; i < beSplit.length; i++) {
        ...
        let fourOutExists = typeof fourOut !== 'undefined';
        ...
        translations.forEach(phrase => {
            ...
            } else if (fourOutExists
                && beforePrevExists
                && prevWordExists
                && fourOut.charAt(0).match(/[A-Z]/) 
                && fourOut.charAt(1).match(/[a-z]/)
                && beforePrev === beforePrev.toLowerCase()
                && prevWord === prevWord.toLowerCase()
                && word === word.toLowerCase()
                && fourWords.toLowerCase() === phrase.american) {
                uppercase = false;
                capitalized = false;
                oneOfTwoCapitalized = false;
                oneOfThreeCapitalized = false;
                oneOfFourCapitalized = true;
            } else {
                uppercase = false;
                capitalized = false;
            }
        ...
        function caseMatch(explanation = '', britPhrase, usPhrase, altPhrase = '', secretIndex) {
            ...
            } else if (oneOfFourCapitalized === true) {
                // cap the first letter of the phrase's first word
                britPhrase = firstBritWord[0].toUpperCase() + britPhrase.slice(1);
                usPhrase = usPhrase[0].toUpperCase() + usPhrase.slice(1);
                if (altPhrase.length > 0) {
            ...
            return `<span><div class="context-box"><p><span class="context">Context:</span> ${explanation}</p></div><span><mark class="translated">${britPhrase}</mark><div class="edits-container"><button class="edit" type="button"><img src="./images/icons/stars-stripes.svg"><span>${usPhrase}</span></button><button class="edit" type="button"><img src="./images/icons/union-jack.svg"><span>${altPhrase}</span></button></div><span hidden>${secretIndex}</span></span></span>`;
        }
...
$('#toggle-dialects').on('click', function() {
    // if the american text is visible
    if ($('#ae').css('display') === 'block') {
        ...
        // display the british text
        $('#be').css('display', 'flex');
        // hide the american text
        $('#ae').css('display', 'none');
        ...
        // hide the british text
        $('#be').css('display', 'none');
        // display the american text
...
$('#insult-me').on('click', function() {
    ...
    prevInsult.length > 0 ? $('#prev-arrow').css('display', 'flex') : $('#prev-arrow').css('display', 'none');
